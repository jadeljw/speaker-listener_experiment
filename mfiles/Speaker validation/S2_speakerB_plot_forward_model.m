...
for type_select = 1 : length(data_type)
    file_name =strcat( 'SpeakerB AudioB-',data_type{type_select});
    mkdir(file_name);
    cd(file_name);
    disp(file_name);
    
    %% load data
    load(strcat('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\1-forward model\AudioB_speakerB_forward_',data_type{type_select},'.mat'));
    ...
    for band_select = 1 : length(band_name)
        ...
        save_name1 = strcat('AudioB speakerB forward models plot-',data_type{type_select},'-',band_name{band_select},'.fig');
        suptitle(save_name1(1:end-4));
        saveas(gcf,save_name1);
        
        save_name1 = strcat('AudioB speakerB forward models plot-',data_type{type_select},'-',band_name{band_select},'.jpg');
        saveas(gcf,save_name1);
        close
