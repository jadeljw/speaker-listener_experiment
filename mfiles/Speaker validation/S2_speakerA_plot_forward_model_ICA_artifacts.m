...
%% data type
data_type = {'ICs'};
load('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\0-raw data\ICA\data_speakerA_ICA_select.mat')
...
speaker_chn = data_speakerA_ICA.artifact;
page_number = 1;
...
for type_select = 1 : length(data_type)
    file_name =strcat( 'SpeakerA AudioA-',data_type{type_select},' artifact');
    mkdir(file_name);
    cd(file_name);
    disp(file_name);
    
    %% load data
    load(strcat('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\1-forward model\ICs\AudioA_speakerA_forward_',data_type{type_select},'.mat'));
    ...
    for band_select = 1 : length(band_name)
        ...
        for i = 1 : page_number
            for j = 1 : plot_number
                ...
                if j > 8
                    eval(strcat('subplot(2,2,',num2str(j),')'));
            ...
            save_name1 = strcat('AudioA speakerA forward models plot-',data_type{type_select},'-',band_name{band_select},'-page',num2str(i),'.fig');
            suptitle(save_name1(1:end-4));
            saveas(gcf,save_name1);
            
            save_name1 = strcat('AudioA speakerA forward models plot-',data_type{type_select},'-',band_name{band_select},'-page',num2str(i),'.jpg');
            saveas(gcf,save_name1);
            close
        end
