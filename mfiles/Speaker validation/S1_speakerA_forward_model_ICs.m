...
for type_select = 1 : length(data_type)
    %% load data
    % EEG
%     load(strcat('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\0-raw data\data_speakerA_',data_type{type_select},'.mat'));
    load('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\0-raw data\ICA\data_speakerA_ICs.mat');

    % Audio
    load('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\0-raw data\Audio_A.mat');
    
    % valid mat
    %     load('E:\DataProcessing\speaker-listener_experiment\Speaker Validation\0-raw data\data_speakerA_ICA_select.mat');
    ...
    for band_select = 1 : length(band_name)
        ...
        % calculate
        temp_resp = eval(strcat('data_speakerA_total.',band_name{band_select}));
        temp_stim = eval(strcat('Audio_A_total.',band_name{band_select}));
        ...
        % save result
        data_name = strcat('Audio_speakerA_forward.',band_name{band_select});
        % model
    ...
    save(strcat('AudioA_speakerA_forward_',data_type{type_select},'.mat'),'Audio_speakerA_forward');
end