...
for ii = 1:10%surrogate loop
    ii
    while 1
        if_con = 0;
        rand_seq = randperm(28);
        for i = 1:28
            if(rand_seq(i) == i)
                if_con = 1;
            end
    ...
    for kk = 1:length(freqs)
        ...
        for i = 1:28
            ...
            respA{i} = speakerA_one{rand_seq(i)};
            respAH{i} = speakerAH_one{rand_seq(i)};
            stimB{i} = Audio_B_total.delta{i};
            respB{i} = speakerB_one{rand_seq(i)};
            respBH{i} = speakerBH_one{rand_seq(i)};
...
save res_mTRF_surrogate r_A r_B r_AH r_BH model_A model_B model_AH model_BH
