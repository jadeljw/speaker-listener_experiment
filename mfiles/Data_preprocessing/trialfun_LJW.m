...
cfg_tri.trialdef.eventvalue = [21 22];
...
while tri_cnt <= length(cfg_tri.trl)
    ...
    while cfg_tri.trl(tri_cnt,4) ~= 21
        tri_cnt = tri_cnt + 1;
    end
    if(cfg_tri.trl(tri_cnt,4) == 21)
        %check if there is 3 continuous triggers of 224
        cnt_start = U_trigger_check(cfg_tri.trl(tri_cnt+1:end,4),21);
        if(cnt_start == 2)
            flag_start = 1;
        end
        %check if there is 3 continuous triggers of 144 afterwards
        cnt_end = U_trigger_check(cfg_tri.trl(tri_cnt+cnt_start+1:end,4),22);
        if(cnt_end == 3)
            flag_end = 1;
        end
