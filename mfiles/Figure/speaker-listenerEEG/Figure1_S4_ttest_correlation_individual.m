...
for band_select = 1 : length(band_name)
    ...
    for plot_select = 1 : length(plot_name)
        ...
        for result_name = 1 : length(ttest_result_name)
            ...
            eval(strcat(temp_name,'=zeros(length(chn_area_labels),listener_num,length(timelag));'));
        end
        ...
        for area_select = 1 : length(chn_area_labels)
            for listener_select = 1 : listener_num
                for time_point = 1 : length(timelag)
                    % ttest
                    [H,P,~,STATS] = ttest(data_for_ttest(area_select,listener_select,:,time_point),0,'Tail','right');
                    % result mat
                    for result_name = 1 : length(ttest_result_name)
                        temp_name = strcat('ttest_result.',ttest_result_name{result_name});
                        eval(strcat(temp_name,'(area_select,listener_select,time_point)=',ttest_result_name{result_name},';'));
                    end
                end
            end
        end
        ...
        imagesc(squeeze(sum(ttest_result.H,2)));colorbar;
%         caxis([0 20]);
        colormap('jet');
        xticks(label_select);
