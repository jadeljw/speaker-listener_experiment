...
R_squared_mat_speaker = zeros(length(select_channels),length(timelag));
...
%         cd(type{type_select});
        ...
        for chn_area_select = 1 : length(select_channels)
            chn_file_name = label66{speaker_chn_total(select_channels(chn_area_select))};
            disp(chn_file_name);
            speaker_chn = speaker_chn_total(select_channels(chn_area_select));
            mkdir(chn_file_name);
            cd(chn_file_name);
...
%                    data_name = strcat('mTRF_speakerEEG_listenerEEG_result-',chn_area_labels{chn_area_select},'-timelag',num2str(timelag(time_point)),'ms-',band_file_name,'.mat');
                   ...
                   data_name = strcat('mTRF_speakerEEG_listenerEEG_result-',chn_file_name,'-timelag',num2str(timelag(time_point)),'ms-alpha.mat');
                    ...
                    %                         band_file_name,'\',file_name,'\',select_channels{chn_area_select},'\');
                    
%                     data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Logistic Regression\Speaker ListenerEEG_area zscore\',...
%                         band_file_name,'\',file_name,'\',select_channels{chn_area_select},'\');
                    
                    data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Figure\Speaker-listenerEEG\0-original r value\area\', band_file_name,'\',...
...
%             % imagesc
            ...
            save_name = strcat('Speaker ListenerEEG Logstic Regresssion r^2 Result-',chn_file_name,'-',band_name{band_select},'-imagesc-',type{type_select},'.jpg');
...
%             ylim([0 0.15]);
            save_name1 = strcat('Speaker ListenerEEG Logstic Regresssion r^2 Result-',chn_file_name,'-',band_name{band_select},'-mean-',type{type_select},'.jpg');
            title(save_name1(1:end-4));
            saveas(gcf,save_name1);
            close
%             %
            % save data
            save_name2 = strcat('Audio ListenerEEG Logstic Regresssion r^2 Result-',chn_file_name,'-',band_name{band_select},'-',type{type_select},'-',select_area,'.mat');
        ...
        save_name = strcat('Audio ListenerEEG Logstic Regresssion Large r^2 Result-total-',band_name{band_select},'-imagesc-',type{type_select},'.jpg');
        title(save_name(1:end-4));
        saveas(gcf,save_name);
        close
        ...
        % save data
        save_name = strcat('Audio ListenerEEG Logstic Regresssion r^2 Result-total-',band_name{band_select},'-',type{type_select},'.mat');
