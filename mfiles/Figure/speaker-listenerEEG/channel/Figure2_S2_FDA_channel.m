...
for band_select = 1 : length(band_name)
    ...
    Acc_mat_total = zeros(length(select_channels),listener_num,story_num,length(timelag)); % 1 for correct / 0 for uncorrect
    fisher_value_total =  zeros(length(select_channels),listener_num,story_num,length(timelag));
...
%                 data_name = strcat('mTRF_speakerEEG_listenerEEG_result-',chn_area_labels{chn_area_select},'-timelag',num2str(timelag(time_point)),'ms-',band_file_name,'.mat');
            ...
            xlabel('listener No.');
            ylabel('ACC %');
            save_name = strcat('Speaker ListenerEEG FDR Result-',band_name{band_select},'-imagesc-',chn_file_name,'timelag',num2str(timelag(time_point)),'ms.jpg');
            title(save_name(1:end-4));
            saveas(gcf,save_name);
            
            save_name = strcat('Speaker ListenerEEG FDR Result-',band_name{band_select},'-imagesc-',chn_file_name,'timelag',num2str(timelag(time_point)),'ms.fig');
            saveas(gcf,save_name);
            close
    ...
    save_name = strcat('Speaker ListenerEEG FDR Result-total-',band_name{band_select},'-imagesc.jpg');
    title(save_name(1:end-4));
    saveas(gcf,save_name);
    
    save_name = strcat('Speaker ListenerEEG FDR Result-total-',band_name{band_select},'-imagesc.fig');
    saveas(gcf,save_name);
    close
    
    
    % save data
    save_name = strcat('Speaker ListenerEEG FDR Result-total-',band_name{band_select},'.mat');
