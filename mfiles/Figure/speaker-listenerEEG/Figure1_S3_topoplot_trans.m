...
mkdir('Speaker-listenerEEG topoplot');
cd('Speaker-listenerEEG topoplot')
...
% band_name = {'delta_hilbert', 'theta', 'theta_hilbert'};
...
% range_total  = [0.00660 	0.00822 	0.00352 	0.00596 	0.00415 	0.00740 	0.00970 	0.00450 	0.00990 ];
% range_total  = [0.00660 0.00740 0.00450];% alpha delta theta 99.9%
range_total  = [0.00525 0.00605 0.003861];% alpha delta theta 99.5%
%% new order
...
for band_select = 1 : length(band_name)
    ...
    for plot_select = 1 : length(plot_name)
        ...
        plot_traing_weights_trans = zeros(length(listener_chn),listener_num);
        ...
        if ~isempty(x_ind)
            for ind_select = 1 : length(x_ind)
                ...
                for listener_select = 1 : listener_num
                    ...
                    data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Figure\0-original r value\Speaker-listenerEEG\raw r value with trans\',band_file_name,'\',listener_file_name,'\',chn_area_labels{x_ind(ind_select)});
                    data_name = strcat('mTRF_speakerEEG_listenerEEG_result-',chn_area_labels{x_ind(ind_select)},'-timelag',num2str(timelag(y_ind(ind_select))),'ms-',band_file_name,'.mat');
                    ...
                    if  strcmp(plot_name{plot_select}(1:5),'attend')
                        ...
                        plot_traing_weights_trans(:,listener_select)  = mean(train_mTRF_attend_w_trans_total,3);
                    else
                        plot_traing_weights(:,listener_select)  = mean(train_mTRF_unattend_w_total,3);
                        plot_traing_weights_trans(:,listener_select)  = mean(train_mTRF_unattend_w_trans_total,3);
                    end
...
%                 U_topoplot(abs(mean(plot_traing_weights,2)),layout,label66(listener_chn));
                save_name = strcat(band_file_name,'-',chn_area_labels{x_ind(ind_select)},'-timelag',num2str(timelag(y_ind(ind_select))),'ms.fig');
                suptitle(strcat(save_name(1:end-4),' r=',num2str(data_for_imagesc(x_ind(ind_select),y_ind(ind_select)))));
                
                subplot(121);U_topoplot(abs(mean(plot_traing_weights,2)),layout,label66(listener_chn));title('decoder weights');colorbar('SouthOutside');
                
                subplot(122);U_topoplot(abs(mean(plot_traing_weights_trans,2)),layout,label66(listener_chn));title('transform');colorbar('SouthOutside');
                
                
                saveas(gcf,save_name);
                
                save_name = strcat(band_file_name,'-',chn_area_labels{x_ind(ind_select)},'-timelag',num2str(timelag(y_ind(ind_select))),'ms.jpg');
                ...
                disp(strcat('r=',num2str(data_for_imagesc(x_ind(ind_select),y_ind(ind_select)))));
            end
        end
