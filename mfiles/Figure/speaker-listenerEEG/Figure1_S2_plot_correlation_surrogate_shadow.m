...
for band_select = 1 : length(band_name)
    ...
    for plot_select = 1 : length(plot_name)
        ...
        data_surrogate_temp = zeros(length(surrogate_set),length(chn_area_labels),length(timelag));
        for area_select = 1 : length(chn_area_labels)
            ...
            if plot_area_order(area_select) > 6
                ...
                for surrogate_select  = 1 : length(surrogate_set)
                    data_surrogate_temp(surrogate_select,:,:)  = eval(strcat('data_surrogate_imagesc',num2str(surrogate_set(surrogate_select))));
                end
                hold on;
%                 plot(squeeze(mean(data_surrogate_temp(:,area_select,:))));
                ho = boundedline(timelag, squeeze(mean(data_surrogate_temp(:,area_select,:))), std(squeeze(mean(data_surrogate_temp(:,area_select,:)))), '-k');
                set(ho, 'lineWidth', 0.5, 'color', 'k');
                plot(timelag,data_for_imagesc(area_select,:),'b','LineWidth',2);
...
%                 plot(squeeze(mean(data_surrogate_temp(:,area_select,:))));
        ...
        legend('STD','Surrogate','Real','Location','best');
