...
for band_select = 1 : length(band_name)
    ...
    data_all_type = zeros(plot_num,length(chn_area_labels),length(timelag));
    data_all_type_surrogate = zeros(plot_num,length(surrogate_set),length(chn_area_labels),length(timelag));
...
%             eval(strcat('data_surrogate_imagesc',num2str(surrogate_set(surrogate_select)),'=data_for_imagesc_surrogate;'));
    ...
    save_name = strcat('all type GLM surrogate-',band_name{band_select},'-histogram.jpg');
    suptitle(save_name(1:end-4));
    saveas(gcf,save_name);
    
    save_name = strcat('all type GLM surrogate all type-',band_name{band_select},'-histogram.fig');
    saveas(gcf,save_name);
    close
