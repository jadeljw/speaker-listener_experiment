...
for band_select = 1 : length(band_name)
    ...
    Rsquared_peak_precede_mean = mean(Rsquared_peak_precede,2);
    Rsquared_peak_follow_mean = mean(Rsquared_peak_precede,2);
    ...
    Surrogate_Rsquared_peak_precede_mean = mean(Surrogate_Rsquared_peak_precede,2);
    Surrogate_Rsquared_peak_follow_mean = mean(Surrogate_Rsquared_peak_follow,2);
    
    %% plot histogram
    set(gcf,'outerposition',get(0,'screensize'));
    subplot(121);
    histogram(Rsquared_peak_precede_mean,'Normalization','probability');hold on;histogram(Surrogate_Rsquared_peak_precede_mean,'Normalization','probability');
    ...
    subplot(122);
    histogram(Rsquared_peak_follow_mean,'Normalization','probability');hold on;histogram(Surrogate_Rsquared_peak_follow_mean,'Normalization','probability');
    title(strcat('follow-',band_file_name));
    xlabel('R^2');
    ylabel('Probability');
    legend('Real','Surrogate');
    ...
    temp_mat_precede = sort(Surrogate_Rsquared_peak_precede_mean(:));
    ...
    temp_mat_follow = sort(Surrogate_Rsquared_peak_follow_mean(:));
