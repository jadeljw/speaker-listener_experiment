...
%% surrogate initial
surrogate_times = 1:10;
Surrogate_Rsquared_peak_precede = zeros(length(chn_area_labels),length(chn_area_labels),listener_num);
Surrogate_Rsquared_peak_follow = zeros(length(chn_area_labels),length(chn_area_labels),listener_num);
...
for band_select = 1 : length(band_name)
    ...
    for surrogate_select = 1 : length(surrogate_times)
        for chn_area_select = 1 : length(chn_area_labels)
            ...
            load(strcat('E:\DataProcessing\speaker-listener_experiment\Surrogate\2-logstic Regression\surrogate',num2str(surrogate_select),'\',band_file_name,...
                '\total\',chn_area_labels{chn_area_select},'\Speaker ListenerEEG Logstic Regresssion r^2 Result-',chn_area_labels{chn_area_select},'-',band_file_name,'-total-Small_area.mat'));
            ...
            %% logistic Regression
            Surrogate_Rsquared_peak_precede(surrogate_select,chn_area_select,:) = data_max_value_precede(:,1); % before/after/all
            Surrogate_Rsquared_peak_follow(surrogate_select,chn_area_select,:) = data_max_value_follow(:,1);
    ...
    save(strcat(save_name,'.mat'),'Surrogate_Rsquared_peak_precede','Surrogate_Rsquared_peak_follow');
    
    %% histogram
    set(gcf,'outerposition',get(0,'screensize'));
    subplot(121);
    histogram(Surrogate_Rsquared_peak_precede,'Normalization','probability');
