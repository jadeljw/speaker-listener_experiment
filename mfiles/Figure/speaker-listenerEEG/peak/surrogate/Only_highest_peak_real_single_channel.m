...
for band_select = 1 : length(band_name)
    ...
    for chn_select = 1 : length(speaker_chn_total)
        disp(label66{chn_select});
        speaker_chn = speaker_chn_total(chn_select);
...
%             band_file_name,'\Speaker ListenerEEG Logstic Regresssion r^2 Result-total-',band_file_name,'-total-Small_area.mat'));
        ...
        R_squared_mat = squeeze(R_squared_mat_speaker(chn_select,:,:));
        %% zscore
        ...
        %% logistic Regression
        Rsquared_peak_precede(chn_select,:) = data_max_value_precede(:,1); % before/after/all
        Rsquared_peak_follow(chn_select,:) = data_max_value_follow(:,1);
