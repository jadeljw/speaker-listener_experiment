...
mkdir('raw r value surrogate1');
cd('raw r value surrogate1');
...
%     data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Figure\0-correlation mat\',band_file_name);
    ...
    for listener = 1 : listener_num
        if listener <10
            file_name = strcat('listener10',num2str(listener));
        else
            file_name = strcat('listener1',num2str(listener));
        end
        ...
        for plot_select = 1 : length(plot_name)
            ...
            data_for_imagesc_total = eval(strcat('squeeze(mean(',data_name,',3));'));
            data_for_imagesc =squeeze(data_for_imagesc_total(:,listener,:));
