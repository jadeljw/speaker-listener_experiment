...
mkdir('r value difference');
cd('r value difference')
...
for band_select = 1 : length(band_name)
    ...
    for surrogate_select  = 1 : length(surrogate_set)
        %     data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Figure\0-correlation mat\',band_file_name);
            data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Surrogate\0-raw r value\surrogate',num2str(surrogate_set(surrogate_select)),'\',band_file_name);
    ...
    for plot_select = 1 : length(plot_name)
        ...
        if strcmp(plot_name{plot_select},'attendDecoder')
            data_name_raw = strcat('data_raw.',plot_name{plot_select},'_attend-data_raw.',plot_name{plot_select},'_unattend');
            data_for_imagesc = eval(strcat('squeeze(mean(mean(',data_name_raw,',3),2));'));
        else
            data_name_raw = strcat('data_raw.',plot_name{plot_select},'_unattend-data_raw.',plot_name{plot_select},'_attend');
            data_for_imagesc = eval(strcat('squeeze(mean(mean(',data_name_raw,',3),2));'));
        end
        ...
        for surrogate_select  = 1 : length(surrogate_set)
            if strcmp(plot_name{plot_select},'attendDecoder')
                data_name_surrogate = strcat('data_surrogate',num2str(surrogate_set(surrogate_select)),'.',plot_name{plot_select},'_attend-data_surrogate',num2str(surrogate_set(surrogate_select)),'.',plot_name{plot_select},'_unattend');
                ...
                data_name_surrogate = strcat('data_surrogate',num2str(surrogate_set(surrogate_select)),'.',plot_name{plot_select},'_unattend-data_surrogate',num2str(surrogate_set(surrogate_select)),'.',plot_name{plot_select},'_attend');
                data_for_imagesc_temp  = eval(strcat('squeeze(mean(mean(',data_name_surrogate,',3),2));'));
                eval(strcat('data_surrogate_surrogate',num2str(surrogate_set(surrogate_select)),'=data_for_imagesc_surrogate;'))
            end
        end
            ...
            if plot_area_order(area_select) > 6
                ...
                plot(timelag,data_for_imagesc(area_select,:),'r','LineWidth',2);
                 for surrogate_select  = 1 : length(surrogate_set)
                     ...
                     hold on; plot(timelag,data_surrogate_temp(area_select,:),'b','LineWidth',1);
                 end
        ...
        save_name = strcat('Raw r value difference surrogate-',band_name{band_select},'-plot-',plot_name{plot_select},'.jpg');
