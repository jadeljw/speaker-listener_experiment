                ...
                for j = 1 : length(timelag)
                    ...
                    for train_index = 1 : length(EEGBlock)
                        ...
                        if strcmpi('A',AttendTarget{Shuffle_speaker(train_index)})
                            Audio_attend_train = zscore(data_EEG_speakerA{Shuffle_speaker(train_index)}(speaker_chn,speaker_time_index));
                            Audio_unattend_train= zscore(data_EEG_speakerB{Shuffle_speaker(train_index)}(speaker_chn,speaker_time_index));
                        else
                            Audio_attend_train = zscore(data_EEG_speakerB{Shuffle_speaker(train_index)}(speaker_chn,speaker_time_index));
                            Audio_unattend_train = zscore(data_EEG_speakerA{Shuffle_speaker(train_index)}(speaker_chn,speaker_time_index));
                        end
                    ...
                    for test_index = 1 : length(EEGBlock)
                        ...
                        SpeaekerA_test = zscore(data_EEG_speakerA{Shuffle_speaker(test_index)}(speaker_chn,speaker_time_index));
                        SpeaekerB_test = zscore(data_EEG_speakerB{Shuffle_speaker(test_index)}(speaker_chn,speaker_time_index));
                    ...
                    tic;
                    % save data
                    saveName = strcat('mTRF_speakerEEG_listenerEEG_result-',chn_file_name,'-timelag',num2str(timelag(j)),'ms-',band_file_name,'.mat');
