...
for surrogate_order = 2 : surrogate_time
    surrogate_file_name = strcat('surrogate',num2str(surrogate_order));
...
%                     data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Figure\0-raw r value\Speaker-listenerEEG\raw\',...
%                         band_file_name,'\',file_name,'\',chn_area_labels{chn_area_select},'\');
                    %
                    data_path = strcat('G:\Speaker-listener_experiment\Analysis program\surrogate data\Speaker ListenerEEG zscore surrogate',num2str(surrogate_order),'\',...
                        band_file_name,'\',file_name,'\',chn_area_labels{chn_area_select},'\');
                    ...
                    for story_select = 1:length(attend_target_num)
                        if attend_target_num(story_select) == 1
                            Correlation_mat.attendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerA_corr(story_select);
                            Correlation_mat.attendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerB_corr(story_select);
                            Correlation_mat.unattendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerA_corr(story_select);
                            Correlation_mat.unattendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerB_corr(story_select);
                        else
                            Correlation_mat.attendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerB_corr(story_select);
                            Correlation_mat.attendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerA_corr(story_select);
                            Correlation_mat.unattendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerB_corr(story_select);
                            Correlation_mat.unattendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerA_corr(story_select);
