...
for band_select = 1 : length(band_name)
    ...
    for chn_select = 1 : length(select_channels)
        chn_file_name = label66{speaker_chn_total(select_channels(chn_select))};
        disp(chn_file_name);
        speaker_chn = speaker_chn_total(select_channels(chn_select));
%         mkdir(chn_file_name);
%         cd(chn_file_name);
        ...
        for time_point = 1 : length(timelag)
            ...
            for listener_select = 1 : listener_num
                ...
                data_name = strcat('mTRF_speakerEEG_listenerEEG_result-',chn_file_name,'-timelag',num2str(timelag(time_point)),'ms-',band_file_name,'.mat');
                
                
                data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Figure\Speaker-listenerEEG\0-original r value\area\alpha_FL',...
                    '\',file_name,'\',chn_file_name,'\');
%                 
...
%                     band_file_name,'\',file_name,'\',chn_area_labels{chn_area_select},'\');
                ...
                for story_select = 1:length(attend_target_num)
                    if attend_target_num(story_select) == 1
                        Correlation_mat.attendDecoder_attend(chn_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerA_corr(story_select);
                        Correlation_mat.attendDecoder_unattend(chn_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerB_corr(story_select);
                        Correlation_mat.unattendDecoder_attend(chn_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerA_corr(story_select);
                        Correlation_mat.unattendDecoder_unattend(chn_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerB_corr(story_select);
                    else
                        Correlation_mat.attendDecoder_attend(chn_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerB_corr(story_select);
                        Correlation_mat.attendDecoder_unattend(chn_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerA_corr(story_select);
                        Correlation_mat.unattendDecoder_attend(chn_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerB_corr(story_select);
                        Correlation_mat.unattendDecoder_unattend(chn_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerA_corr(story_select);
...
%     ylabels = label66{speaker_chn_total(select_channels)};
