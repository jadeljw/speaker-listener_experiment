...
for band_select = 1 : length(band_name)
    ...
    for listener_select = 1 : listener_num
        ...
        data_max_select(listener_select,data_max_index_precede(listener_select,2)) = 1;
    end
    ...
    for listener_select = 1 : listener_num
        ...
        data_name = strcat('mTRF_Audio_listenerEEG_result-timelag',num2str(timelag(data_max_index_precede(listener_select,2))),'ms-',band_name{band_select},'.mat');
        load(strcat(data_path,data_name));
    ...
    save_name = strcat('Highest R^2 value weights precede-',band_file_name);
    suptitle(save_name);
    saveas(gcf,strcat(save_name,'.jpg'));
    saveas(gcf,strcat(save_name,'.fig'));
    close
    ...
    save_name = strcat('Highest R^2 value trans weights precede-',band_file_name);
    suptitle(save_name);
    saveas(gcf,strcat(save_name,'.jpg'));
    saveas(gcf,strcat(save_name,'.fig'));
