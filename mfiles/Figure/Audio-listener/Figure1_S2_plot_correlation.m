...
for band_select = 1 : length(band_name)
    ...
    for listener_select = 1 : listener_num
        for plot_select = 1 : length(plot_name)
            ...
            data_for_imagesc = eval(strcat('squeeze(mean(',data_name_raw,',2));'));
            ...
            plot(timelag,data_for_imagesc,'b','LineWidth',2);
...
%             legend('Real','Surrogate','Location','best');
        ...
        save_name = strcat('Audio listenerEEG-',band_name{band_select},'-listener No.',num2str(listener_select),'-plot.fig');
        saveas(gcf,save_name);
        close
