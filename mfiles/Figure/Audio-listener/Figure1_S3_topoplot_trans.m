...
for band_select = 1 : length(band_name)
    ...
    for plot_select = 1 : length(plot_name)
        ...
        x_ind = ones(1,length(timelag)) .*2;
        y_ind = 1:length(timelag);
        ...
        if ~isempty(y_ind)
            for ind_select = 1 : length(unique(y_ind))
                ...
                suptitle(strcat(save_name(1:end-4),' r_S_L=',num2str(data_for_imagesc(x_ind(ind_select),y_ind(ind_select))),...
