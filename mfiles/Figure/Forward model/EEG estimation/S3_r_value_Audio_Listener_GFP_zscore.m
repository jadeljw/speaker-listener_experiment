...
for band_select = 1 : length(band_name)
    ...
    for time_select = 1 : length(start_time_total)
        ...
        for listener_select = 1 : 20
            ...
            for time_point = 1 : timelag_length
                for chn_select = 1 : length(channel_select_total)
                    GFP_attend(listener_select,time_point) =  data_att_select(time_point,chn_select).^2 + GFP_attend(listener_select,time_point);
                    GFP_unattend(listener_select,time_point) =  data_unatt_select(time_point,chn_select).^2 + GFP_unattend(listener_select,time_point);
                end
            end
            ...
            xticks(1:(length(GFP_attend)-1)/xticks_num:length(GFP_attend));
        ...
        xlabel('timelag(ms)');
        ylabel('amplitude');
        title(strcat('GFP Zscore start-time:',num2str(start_time_total(time_select)),'ms~end-time:',num2str(end_time_total(time_select)),'ms-',band_file_name));
        save_name = strcat('GFP Zscore start-time ',num2str(start_time_total(time_select)),'ms~end-time ',num2str(end_time_total(time_select)),'ms-',band_file_name);
        saveas(gcf,strcat(save_name,'.jpg'));
        saveas(gcf,strcat(save_name,'.fig'));
        close
