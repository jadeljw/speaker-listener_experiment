...
for band_select = 1 : length(band_name)
    ...
    for chn_select = 1 :length(listener_chn)
        plot(timelag,squeeze(mean(train_mTRF_attend_w_mean(:,chn_select,:),1)),'lineWidth',2);
        hold on;plot(timelag,squeeze(mean(train_mTRF_unattend_w_mean(:,chn_select,:),1)),'lineWidth',2);
        
        save_name = strcat(label66{listener_chn(chn_select)});
        title(save_name);
