...
%% band name
lambda = 2^5;
band_name = strcat(' 64Hz 2-8Hz keep order Listner00 lambda',num2str(lambda),' 10-55s');
...
load('E:\DataProcessing\speaker-listener_experiment\AudioData\from EEG\0-LZR-Listener-hilbert-sound_64Hz_lowpass8Hz.mat');
...
for j = 1 : length(timelag)
    ...
    for test_index = 1 : length(EEGBlock)
        ...
        if strcmpi(Space(test_index),'left')
            ...
            Audio_unattend_test = audio_EEGBlock{test_index}(2,EEG_time);
        else
            Audio_attend_test = audio_EEGBlock{test_index}(2,EEG_time);
        ...
        for train_index = 1 : length(EEGBlock)
            if train_index ~= test_index
                ...
                if strcmpi(Space(train_index),'left')
                    ...
                    Audio_unattend_train= audio_EEGBlock{train_index}(2,EEG_time);
                else
                    Audio_attend_train = audio_EEGBlock{train_index}(2,EEG_time);
