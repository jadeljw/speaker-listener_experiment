...
for band_select = 2
    ...
    for i = 1 : 20
        ...
        %% extrcat attend target A trial
        attend_target_char = cell2mat(AttendTarget);
        traget_index = find(attend_target_char=='B');
        % Audio
        data_left = data_left(traget_index);
        data_right = data_right(traget_index);
        % EEG
        EEGBlock = EEGBlock(traget_index);
        %% timelag
        Fs = 64;
...
%             MSE_recon_UnattendDecoder_attend_corr_train =  zeros(size(EEGBlock,1)-1,size(EEGBlock,1));
            ...
            for train_index = 1 : length(EEGBlock)
                ...
                if strcmpi(Space(train_index),'left')
                    ...
                    EEGBlock{train_index} = EEGBlock{train_index}(chn_re_index,:);
                    EEG_train =  EEGBlock{train_index}(listener_chn,EEG_time);
                end
            ...
            for test_index = 1 : length(EEGBlock)
                ...
                if strcmpi(Space(test_index),'left')
                    ...
                    EEGBlock{test_index} = EEGBlock{test_index}(chn_re_index,:);
                    EEG_test =  EEGBlock{test_index}(listener_chn,EEG_time);
                end
