...
% attend stream are from left


%% band name
lambda = 2^5;
band_name = strcat(' 64Hz 2-8Hz sound from EEG lambda',num2str(lambda),' 10-65s unattend only');
...
% write into cell
AudioA_total = [AudioA_retell_cell(1:6) AudioA_read_cell(1:8)];
AudioB_total = [AudioB_retell_cell(1:6) AudioB_read_cell(1:8)];
...
% EEGBlock = EEGBlock';
audio_EEGBlock = [data_retell(7:14) data_read(9:14)];
...
% combine
EEGBlock = [data_retell(7:14) data_read(9:14)];
EEGBlock = EEGBlock';
...
AttendTarget = [AttendTarget(7:14);AttendTarget(23:28)];
%% timelag
Fs = 64;
