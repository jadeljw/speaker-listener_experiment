...
for band_select = 2
    ...
    for i = 1 : 20
        ...
        %% band name
        lambda = 2^5;
        bandName = strcat(' 64Hz',band_name{band_select},' sound from wav Listener',dataName(1:3),' lambda',num2str(lambda),' 10-55s');
        ...
        Audio_name_left = strcat('data_left_',band_name{band_select});
        Audio_name_right = strcat('data_right_',band_name{band_select});
        load(strcat('E:\DataProcessing\speaker-listener_experiment\AudioData\from wav\Listener',dataName(1:3),'_Audio_envelope_hilbert_first_64Hz_keep_order.mat'),...
            Audio_name_left,Audio_name_right);
        data_left = eval(Audio_name_left);
        data_right = eval(Audio_name_right);
        ...
        EEG_name = strcat('data_filtered_',band_name{band_select});
        load(strcat('E:\DataProcessing\speaker-listener_experiment\ListenerData\',dataName,'.mat'),EEG_name);
        
        
        % combine
        EEGBlock = eval(strcat(EEG_name,'.trial'));
        EEGBlock = EEGBlock';
        ...
        for j = 1 : length(timelag)
            ...
            fisher_feature_test_Attend =  zeros(2,1);
            fisher_feature_test_Unattend =  zeros(2,1);
            %             fisher_label_test = zeros(2,1);
            
            fisher_feature_train_Attend =  zeros(2,size(EEGBlock,1)-1);
            fisher_feature_train_Unattend =  zeros(2,size(EEGBlock,1)-1);
            fisher_label_train = zeros(1,size(EEGBlock,1)-1);
            ...
            for test_index = 1 : length(EEGBlock)
                ...
                if strcmpi(Space(test_index),'left')
                    ...
                    % test EEG
                    EEG_test =  EEGBlock{test_index}(listener_chn,EEG_time);
                else
                    Audio_attend_test = data_right{test_index}(Audio_time)';
                    Audio_unattend_test = data_left{test_index}(Audio_time)';
                    % test EEG and reverse
                    EEG_test = EEGBlock{test_index}(chn_re_index,:);
                    EEG_test = EEG_test(listener_chn,EEG_time);
                end
                
                % test label
                if strcmpi(AttendTarget(test_index),'A')
                    fisher_label_test = 1;
                else
                    fisher_label_test = 2;
                end
                ...
                fisher_feature_test_Attend = [recon_AttendDecoder_attend_corr(test_index); recon_AttendDecoder_unattend_corr(test_index)];
                fisher_feature_test_Unattend = [recon_UnattendDecoder_unattend_corr(test_index); recon_UnattendDecoder_attend_corr(test_index)];
                
                % cross validation
                cnt_crossval = 1;
                for cross_val_index = train_index_select
                    
%                     disp(strcat('Testing story',num2str(test_index),'...cross validation'));
                    
                    % cross_val data
                    if strcmpi(Space(cross_val_index),'left')
                        Audio_attend_crossval = data_left{cross_val_index}(Audio_time)';
                        Audio_unattend_crossval = data_right{cross_val_index}(Audio_time)';
                        % test EEG
                        EEG_crossval =  EEGBlock{cross_val_index}(listener_chn,EEG_time);
                    else
                        Audio_attend_crossval = data_right{cross_val_index}(Audio_time)';
                        Audio_unattend_crossval = data_left{cross_val_index}(Audio_time)';
                        % test EEG and reverse
                        EEG_crossval = EEGBlock{cross_val_index}(chn_re_index,:);
                        EEG_crossval =  EEG_crossval(listener_chn,EEG_time);
                    end
                    
                    % cross validation
                    [recon_audio_attend_AttendDecoder,recon_AttendDecoder_attend_corr_train(cnt_crossval,test_index),p_recon_AttendDecoder_attend_corr_train(cnt_crossval,test_index),MSE_recon_AttendDecoder_attend_corr(test_index)] =...
                        mTRFpredict(Audio_attend_crossval',EEG_crossval',train_mTRF_attend_w_mean,Fs,-1,start_time,end_time,train_mTRF_attend_con_mean);
                    
                    [recon_audio_unattend_AttendDecoder,recon_AttendDecoder_unattend_corr_train(cnt_crossval,test_index),p_recon_AttendDecoder_unattend_corr_train(cnt_crossval,test_index),MSE_recon_AttendDecoder_unattend_corr(test_index)] =...
                        mTRFpredict(Audio_unattend_crossval',EEG_crossval',train_mTRF_attend_w_mean,Fs,-1,start_time,end_time,train_mTRF_attend_con_mean);
                    
                    [recon_audio_unattend_unAttendDecoder,recon_UnattendDecoder_unattend_corr_train(cnt_crossval,test_index),p_recon_UnattendDecoder_unattend_corr_train(cnt_crossval,test_index),MSE_recon_UnattendDecoder_unattend_corr(test_index)] =...
                        mTRFpredict(Audio_unattend_crossval',EEG_crossval',train_mTRF_unattend_w_mean,Fs,-1,start_time,end_time,train_mTRF_unattend_con_mean);
                    
                    [recon_audio_attend_unAttendDecoder,recon_UnattendDecoder_attend_corr_train(cnt_crossval,test_index),p_recon_UnattendDecoder_attend_corr_train(cnt_crossval,test_index),MSE_recon_UnattendDecoder_attend_corr(test_index)] =...
                        mTRFpredict(Audio_attend_crossval',EEG_crossval',train_mTRF_unattend_w_mean,Fs,-1,start_time,end_time,train_mTRF_unattend_con_mean);
                    
                    
                    % record label
                    if strcmpi(AttendTarget(cross_val_index),'A')
                        fisher_label_train(cnt_crossval) = 1;
                    else
                        fisher_label_train(cnt_crossval) = 2;
                    end
                    
                    % cnt
                    cnt_crossval =   cnt_crossval + 1;
                    
                    
                    % record r value
                    fisher_feature_train_Attend =  [recon_AttendDecoder_attend_corr_train(:,test_index)';recon_AttendDecoder_unattend_corr_train(:,test_index)'];
                    fisher_feature_train_Unattend =  [recon_UnattendDecoder_unattend_corr_train(:,test_index)';recon_UnattendDecoder_attend_corr_train(:,test_index)'];
                end
                
                % recored into mat for fisher
                fisher_feature_train_Attend_all_story{test_index} = fisher_feature_train_Attend;
                fisher_feature_train_Unattend_all_story{test_index} = fisher_feature_train_Unattend;
                fisher_feature_test_Attend_all_story{test_index} = fisher_feature_test_Attend;
                fisher_feature_test_Unattend_all_story{test_index} = fisher_feature_test_Unattend;
                fisher_label_train_all_story{test_index} = fisher_label_train;
                fisher_label_test_all_story{test_index} =  fisher_label_test;
            ...
            %             saveName1 = strcat('Reconstruction Accuracy using mTRF method for attend decoder+',num2str(timelag(j)),'ms',bandName,'.jpg');
            ...
            %             xlabel('Subject No.'); ylabel('r value')
            %             saveName2 = strcat('Reconstruction Accuracy using mTRF method for unattend decoder+',num2str(timelag(j)),'ms',bandName,'.jpg');
            ...
            %             saveName3 = strcat('Decoding accuracy using mTRF method for attend and unattend decoder+',num2str(timelag(j)),'ms',bandName,'.jpg');
            ...
            save(saveName,'recon_AttendDecoder_attend_corr','recon_UnattendDecoder_unattend_corr' ,'recon_AttendDecoder_unattend_corr','recon_UnattendDecoder_attend_corr',...
                ...
                'fisher_feature_train_Attend_all_story','fisher_feature_train_Unattend_all_story','fisher_label_train_all_story',...
                'fisher_feature_test_Attend_all_story','fisher_feature_test_Unattend_all_story','fisher_label_test_all_story');
