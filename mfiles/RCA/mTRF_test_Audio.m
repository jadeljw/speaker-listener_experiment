...
for band_select = 1 : length(band_name)
    ...
    for listener_select = 1 : listener_num
        ...
        load('E:\DataProcessing\speaker-listener_experiment\Demo data\20180605\Audio_A_total.mat');
        load('E:\DataProcessing\speaker-listener_experiment\Demo data\20180605\Audio_B_total.mat');
        data_EEG_AudioA = eval(strcat('Audio_A_total.',band_file_name));
        data_EEG_AudioB = eval(strcat('Audio_B_total.',band_file_name));
        ...
        for j = 1 : length(timelag)
            ...
            for train_index = 1 : size(EEGBlock,3)
                ...
                if  attend_target_num(train_index) == 1 % attend A
                    Audio_attend_train = data_EEG_AudioA{train_index};
                    Audio_unattend_train= data_EEG_AudioB{train_index};
                else
                    Audio_attend_train = data_EEG_AudioB{train_index};
                    Audio_unattend_train = data_EEG_AudioA{train_index};
                end
            ...
            for test_index = 1 : size(EEGBlock,3)
                ...
                SpeaekerA_test = data_EEG_AudioA{test_index};
                SpeaekerB_test = data_EEG_AudioB{test_index};
            ...
            tic;
            % save data
            saveName = strcat('mTRF_Audio_listenerEEG_RCA_result-timelag',num2str(timelag(j)),'ms-',band_file_name,'.mat');
