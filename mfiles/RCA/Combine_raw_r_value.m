...
%                 data_name = strcat('mTRF_speaker_listenerEEG_RCA_result-timelag',num2str(timelag(time_point)),'ms-',band_file_name,'.mat');
%                 
%                 
%                 data_path = strcat('E:\DataProcessing\speaker-listener_experiment\mfiles\RCA\Speaker-listener\mTRF\',...
%                     band_file_name,'\',file_name,'\');
                ...
                for story_select = 1:length(attend_target_num)
                    if attend_target_num(story_select) == 1
                        Correlation_mat.attendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerA_corr(story_select,chn_area_select);
                        Correlation_mat.attendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerB_corr(story_select,chn_area_select);
                        Correlation_mat.unattendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerA_corr(story_select,chn_area_select);
                        Correlation_mat.unattendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerB_corr(story_select,chn_area_select);
                    else
                        Correlation_mat.attendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerB_corr(story_select,chn_area_select);
                        Correlation_mat.attendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_AttendDecoder_SpeakerA_corr(story_select,chn_area_select);
                        Correlation_mat.unattendDecoder_attend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerB_corr(story_select,chn_area_select);
                        Correlation_mat.unattendDecoder_unattend(chn_area_select,listener_select,story_select,time_point) = recon_UnattendDecoder_SpeakerA_corr(story_select,chn_area_select);
    ...
    for plot_select = 1 : length(plot_name)
        ...
        save_name = strcat('Raw r value-',band_name{band_select},'-imagesc-',plot_name{plot_select},'.jpg');
