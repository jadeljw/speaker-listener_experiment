...
bandName = ' 2Hz-8Hz 64Hz';

%%  mTRF plot
% p = pwd;
p = 'E:\DataProcessing\speaker-listener_experiment\Decoding Result\CCA-sound-EEG\listener0';
% category = 'mTRF';
...
for  j = 1 : length(timelag)
    ...
    dataName = strcat('cca_sound_EEG_result+',num2str(timelag(j)),'ms',bandName,'.mat');
...
xlabel('Times(ms)');
ylabel('r-value')
saveName1 = strcat( 'Attended decoder Reconstruction-Acc across all time-lags using CCA sound EEG method',bandName,'.jpg');
...
xlabel('Times(ms)');
ylabel('r-value')
saveName2 = strcat('Unattended decoder Reconstruction-Acc across all time-lags using CCA sound EEG method',bandName,'.jpg');
...
saveName3 =strcat('Decoding-Accuracy across all time-lags using CCA sound EEG method',bandName,'.jpg');
