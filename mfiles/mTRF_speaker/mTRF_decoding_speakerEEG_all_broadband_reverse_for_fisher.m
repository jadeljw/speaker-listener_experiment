...
for i = 1 : 20
    ...
    if strcmp(Type{1},'reading')
        % reading part first
        data_EEG_speakerA = [data_speakerA_read_broadband_valid(speaker_story_read_order) data_speakerA_retell_broadband_valid(speaker_story_retell_order)];
        data_EEG_speakerB = [data_speakerB_read_broadband_valid(speaker_story_read_order) data_speakerB_retell_broadband_valid(speaker_story_retell_order)];
    else
        % retelling part first
        data_EEG_speakerA = [data_speakerA_retell_broadband_valid(speaker_story_retell_order) data_speakerA_read_broadband_valid(speaker_story_read_order)];
        data_EEG_speakerB = [data_speakerB_retell_broadband_valid(speaker_story_retell_order) data_speakerB_read_broadband_valid(speaker_story_read_order)];
        ...
        for j = 1 : length(timelag)
            ...
            for test_index = 1 : length(EEGBlock)
                ...
                for cross_val_index = train_index_select
                    ...
                    %                         EEGBlock{cross_val_index} = EEGBlock{cross_val_index}(chn_re_index,:);
                    %                         EEG_crossval =  EEGBlock{cross_val_index}(listener_chn,EEG_time);
                    %                     end
                    %
                    
                    if  strcmpi('A',AttendTarget{cross_val_index})
                        Audio_attend_crossval = data_EEG_speakerA{cross_val_index}(speaker_chn(chn),speaker_time_index);
                        Audio_unattend_crossval = data_EEG_speakerB{cross_val_index}(speaker_chn(chn),speaker_time_index);
                    else
                        Audio_attend_crossval = data_EEG_speakerB{cross_val_index}(speaker_chn(chn),speaker_time_index);
                        Audio_unattend_crossval = data_EEG_speakerA{cross_val_index}(speaker_chn(chn),speaker_time_index);
                    end
                    
                    % test EEG
                    if strcmpi(Space(test_index),'left')
                        EEG_crossval =  EEGBlock{cross_val_index}(listener_chn,Listener_time_index);
                    else
                        EEG_crossval = EEGBlock{cross_val_index}(chn_re_index,:);
                        EEG_crossval =  EEG_crossval(listener_chn,Listener_time_index);
                    end
