...
for i = 1 : listener_num
    ...
    for chn = 1:length(speaker_chn)
        ...
        decoding_acc_attended_abs = zeros(1,length(timelag));
        decoding_acc_unattended_abs = zeros(1,length(timelag));
...
%         cd('topoplot');
        for  j = 1 : length(timelag)
            ...
            %decoding accuracy ABS
            Decoding_result_attend_decoder_abs = abs(recon_AttendDecoder_attend_corr)-abs(recon_AttendDecoder_unattend_corr);
            Individual_subjects_result_attend = mean(sum(Decoding_result_attend_decoder_abs>0)/length(Decoding_result_attend_decoder_abs));
            decoding_acc_attended_abs(j)= Individual_subjects_result_attend;
            
            Decoding_result_unattend_decoder_abs = abs(recon_UnattendDecoder_unattend_corr)-abs(recon_UnattendDecoder_attend_corr);
            Individual_subjects_result_unattend = mean(sum(Decoding_result_unattend_decoder_abs>0)/length(Decoding_result_unattend_decoder_abs));
            decoding_acc_unattended_abs(j) = Individual_subjects_result_unattend;
        ...
        xlabel('Times(ms)');
        ylabel('Decoding accuracy(%)')
        saveName3 =strcat('Decoding-Accuracy mTRF_speakerEEG_listenerEEG_ABS_result-',label66{speaker_chn(chn)},'.jpg');
        title(saveName3(1:end-4));
        legend('Attended decoder','Unattended decoder','Location','northeast');ylim([30,100]);
        saveas(gcf,saveName3);
        close
        ...
        decoding_acc_unattended_all_listener(i,chn,:) = decoding_acc_unattended;
         decoding_acc_attended_abs_all_listener(i,chn,:) = decoding_acc_attended;
        decoding_acc_unattended_abs_all_listener(i,chn,:) = decoding_acc_unattended;
        ...
        save(saveName,'decoding_acc_attended','decoding_acc_unattended',...
            ...
            'decoding_acc_attended_abs','decoding_acc_unattended_abs');
