%% speakerEEG mTRF backward model
...
mkdir('theta reverse long');
cd('theta reverse long');
 ...
 %% length
    ...
    for chn = 1 : length(speaker_chn)
        ...
        for j = 1 : length(timelag)
            ...
            %% save data
             saveName = strcat('mTRF_speakerEEG_listenerEEG_backward_result+',label66{speaker_chn(chn)},'-timelag',num2str(timelag(j)),'ms.mat');
