...
mkdir('broadband reverse P4');
cd('broadband reverse P4');
...
for lambda_num = 1 : length(lambda_para)
    ...
    for i = 1 : listener_num
        ...
        band_name = strcat(' 64Hz broadband speakerEEG mTRF Listener',file_name(end-2:end),' lambda',num2str(lambda),' 10-55s');
        ...
        for chn = 1:length(speaker_chn)
            ...
            % p = pwd;
            p =strcat('E:\DataProcessing\speaker-listener_experiment\Decoding Result\mTRF_speaker\Listener-Speaker\broadband reverse\lambda\lambda',num2str(lambda),'\',file_name);
            % category = 'mTRF';
