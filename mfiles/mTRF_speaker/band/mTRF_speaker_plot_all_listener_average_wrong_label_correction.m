...
for chn = 1:length(speaker_chn)
    ...
    % channel name
    chn_file_name = strcat(num2str(chn),'-',label66{chn});
    mkdir(chn_file_name);
    cd(chn_file_name);
    ...
    for i = 1 : listener_num
        ...
        %% load data
        datapath = strcat('E:\DataProcessing\speaker-listener_experiment\Plot\timelag plot\mTRF_speaker\标签标错的其实算的都是前额的数据\broadband\',file_name,'\',num2str(chn),'-',label66{speaker_chn(chn)});
...
%         decoding_acc_unattended_abs_all_listener(i,:) = decoding_acc_unattended_abs;
    ...
    saveName3 =strcat('All listener Mean Decoding-Acc across timelags using mTRF-Speaker method-',chn_file_name,'.jpg');
