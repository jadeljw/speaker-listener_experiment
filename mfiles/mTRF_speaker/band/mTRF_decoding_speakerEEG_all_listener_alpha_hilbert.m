...
for i = 9 : 20
    ...
    %% band name
    lambda = 2^5;
    band_name = strcat(' 64Hz alpha hilbert speakerEEG mTRF Listener',dataName(1:3),' lambda',num2str(lambda),' 10-55s');
    ...
    if strcmp(Type{1},'reading')
        % reading part first
        data_EEG_speakerA = [data_speakerA_read_alpha_valid(speaker_story_read_order) data_speakerA_retell_alpha_valid(speaker_story_retell_order)];
        data_EEG_speakerB = [data_speakerB_read_alpha_valid(speaker_story_read_order) data_speakerB_retell_alpha_valid(speaker_story_retell_order)];
    else
        % retelling part first
        data_EEG_speakerA = [data_speakerA_retell_alpha_valid(speaker_story_retell_order) data_speakerA_read_alpha_valid(speaker_story_read_order)];
        data_EEG_speakerB = [data_speakerB_retell_alpha_valid(speaker_story_retell_order) data_speakerB_read_alpha_valid(speaker_story_read_order)];
    ...
    EEGBlock = data_filtered_alpha.trial;
    EEGBlock = EEGBlock';
