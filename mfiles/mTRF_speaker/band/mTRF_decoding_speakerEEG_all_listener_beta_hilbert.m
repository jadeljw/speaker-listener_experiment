...
for i = 1 : 20
    ...
    %% band name
    lambda = 2^5;
    band_name = strcat(' 64Hz beta hilbert speakerEEG mTRF Listener',dataName(1:3),' lambda',num2str(lambda),' 10-55s');
    ...
    load('E:\DataProcessing\speaker-listener_experiment\SpeakerData\Speaker01-CFY-read_retell_valid.mat',...
        'data_speakerA_retell_beta_valid','data_speakerA_read_beta_valid');
    load('E:\DataProcessing\speaker-listener_experiment\SpeakerData\Speaker02-FS-read_retell_valid.mat',...
        'data_speakerB_retell_beta_valid','data_speakerB_read_beta_valid');
    
    
    if strcmp(Type{1},'reading')
        % reading part first
        data_EEG_speakerA = [data_speakerA_read_beta_valid(speaker_story_read_order) data_speakerA_retell_beta_valid(speaker_story_retell_order)];
        data_EEG_speakerB = [data_speakerB_read_beta_valid(speaker_story_read_order) data_speakerB_retell_beta_valid(speaker_story_retell_order)];
    else
        % retelling part first
        data_EEG_speakerA = [data_speakerA_retell_beta_valid(speaker_story_retell_order) data_speakerA_read_beta_valid(speaker_story_read_order)];
        data_EEG_speakerB = [data_speakerB_retell_beta_valid(speaker_story_retell_order) data_speakerB_read_beta_valid(speaker_story_read_order)];
        ...
        for j = 1 : length(timelag)
            ...
            for train_index = 1 : length(EEGBlock)
                ...
                if strcmpi('A',AttendTarget{train_index})
                    Audio_attend_train = abs(hilbert(data_EEG_speakerA{train_index}(speaker_chn(chn),speaker_time_index)));
                    Audio_unattend_train= abs(hilbert(data_EEG_speakerB{train_index}(speaker_chn(chn),speaker_time_index)));
                else
                    Audio_attend_train = abs(hilbert(data_EEG_speakerB{train_index}(speaker_chn(chn),speaker_time_index)));
                    Audio_unattend_train = abs(hilbert(data_EEG_speakerA{train_index}(speaker_chn(chn),speaker_time_index)));
                end
                
                % train EEG
                EEG_train =  abs(hilbert(EEGBlock{train_index}(listener_chn,Listener_time_index)));
            ...
            for test_index = 1 : length(EEGBlock)
                ...
                if  strcmpi('A',AttendTarget{test_index})
                    Audio_attend_test = abs(hilbert(data_EEG_speakerA{test_index}(speaker_chn(chn),speaker_time_index)));
                    Audio_unattend_test = abs(hilbert(data_EEG_speakerB{test_index}(speaker_chn(chn),speaker_time_index)));
                else
                    Audio_attend_test = abs(hilbert(data_EEG_speakerB{test_index}(speaker_chn(chn),speaker_time_index)));
                    Audio_unattend_test = abs(hilbert(data_EEG_speakerA{test_index}(speaker_chn(chn),speaker_time_index)));
                end
                
                % EEG
                EEG_test =  abs(hilbert(EEGBlock{test_index}(listener_chn,Listener_time_index)));
