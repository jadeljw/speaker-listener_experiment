...
mkdir('1_8Hz reverse zscore');
cd('1_8Hz reverse zscore');
...
for i = 1 : listener_num
    ...
    %% band name
    lambda = 2^10;
    %     band_name = strcat(' 64Hz 1_8Hz speakerEEG mTRF Listener',dataName(1:3),' lambda',num2str(lambda),' 10-55s');
    ...
    load('E:\DataProcessing\speaker-listener_experiment\SpeakerData\Speaker01-CFY-read_retell_valid_strict_1-8Hz.mat',...
        'data_speakerA_retell_1_8Hz_valid','data_speakerA_read_1_8Hz_valid');
    load('E:\DataProcessing\speaker-listener_experiment\SpeakerData\Speaker02-FS-read_retell_valid_strict_1-8Hz.mat',...
        'data_speakerB_retell_1_8Hz_valid','data_speakerB_read_1_8Hz_valid');
    
    
    if strcmp(Type{1},'reading')
        % reading part first
        data_EEG_speakerA = [data_speakerA_read_1_8Hz_valid(speaker_story_read_order) data_speakerA_retell_1_8Hz_valid(speaker_story_retell_order)];
        data_EEG_speakerB = [data_speakerB_read_1_8Hz_valid(speaker_story_read_order) data_speakerB_retell_1_8Hz_valid(speaker_story_retell_order)];
    else
        % retelling part first
        data_EEG_speakerA = [data_speakerA_retell_1_8Hz_valid(speaker_story_retell_order) data_speakerA_read_1_8Hz_valid(speaker_story_read_order)];
        data_EEG_speakerB = [data_speakerB_retell_1_8Hz_valid(speaker_story_retell_order) data_speakerB_read_1_8Hz_valid(speaker_story_read_order)];
    ...
    %% load EEG data
    load(strcat('E:\DataProcessing\speaker-listener_experiment\ListenerData\',dataName,'.mat'),'data_filtered_1_8Hz');
    
    % combine
    %     EEGBlock = data_filtered_1_8Hz.trial;
    EEGBlock = data_filtered_1_8Hz.trial;
    EEGBlock = EEGBlock';
