...
for band_select = 1 : length(band_name)
    ...
    for i = 1 : 20
        ...
        % data name
        data_speakerA_retell_name = strcat('data_speakerA_retell_',band_name{band_select},'_valid');
        data_speakerA_read_name = strcat('data_speakerA_read_',band_name{band_select},'_valid');
        data_speakerB_retell_name = strcat('data_speakerB_retell_',band_name{band_select},'_valid');
        data_speakerB_read_name = strcat('data_speakerB_read_',band_name{band_select},'_valid');
        
        
        % load data
        load('E:\DataProcessing\speaker-listener_experiment\SpeakerData\Speaker01-CFY-read_retell_valid_strict_1-8Hz.mat',...
            data_speakerA_retell_name,data_speakerA_read_name);
        load('E:\DataProcessing\speaker-listener_experiment\SpeakerData\Speaker02-FS-read_retell_valid_strict_1-8Hz.mat',...
            data_speakerB_retell_name,data_speakerB_read_name);
        
        
        % data eval
        data_speakerA_retell = eval(data_speakerA_retell_name);
        data_speakerA_read =  eval(data_speakerA_read_name);
        data_speakerB_retell = eval(data_speakerB_retell_name);
        data_speakerB_read = eval(data_speakerB_read_name);
        ...
        %% load EEG data
        data_EEG_name = strcat('data_filtered_',band_name{band_select});
        load(strcat('E:\DataProcessing\speaker-listener_experiment\ListenerData\',dataName,'.mat'),data_EEG_name);
        
        % combine
        %     EEGBlock = data_filtered_delta.trial;
        EEGBlock = eval(strcat(data_EEG_name,'.trial'));
        EEGBlock = EEGBlock';
        ...
        for chn = 1 : length(speaker_chn)
            ...
            for j = 1 : length(timelag)
                ...
                end_time = 6000 + timelag(j);
                
                
                %% mTRF train and test
                tic;
                [R_attend,P_attend,MSE_attend,~,model_attend,model_reshape_attend] = mTRFcrossval_reshape(Speaker_EEG_attend,Listener_EEG_test,Fs,1,start_time,end_time,lambda);
                [R_unattend,P_unattend,MSE_unattend,~,model_unattend,model_reshape_unattend] = mTRFcrossval_reshape(Speaker_EEG_unattend,Listener_EEG_test,Fs,1,start_time,end_time,lambda);
                
                %% save data
                saveName = strcat('mTRF_speakerEEG_listenerEEG_forward_result+',label66{speaker_chn(chn)},'-',band_file_name,'.mat');
                %     saveName = strcat('mTRF_sound_EEG_result.mat');
                save(saveName,'R_attend','P_attend','MSE_attend','model_reshape_attend',...
                    'R_unattend','P_unattend','MSE_unattend','model_reshape_unattend');
                toc;
            end
