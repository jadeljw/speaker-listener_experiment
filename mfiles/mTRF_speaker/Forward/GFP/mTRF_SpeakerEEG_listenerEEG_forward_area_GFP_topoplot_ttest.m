...
chn_area_labels = fieldnames(Large_area);
...
for chn_area_select = 1 : length(chn_area_labels)
    ...
    speaker_chn = eval(strcat('Large_area.',chn_area_labels{chn_area_select}));
            ...
            for lambda_select  = 1 : length(lambda_index)
                ...
                for i = 1 : listener_num
                    ...
                    data_name = strcat('mTRF_speakerEEG_listenerEEG_forward_result+',label66{speaker_chn(chn_speaker)},'-',band_name{band_select},'.mat');
                                        data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Forward model\SpeakerEEG-listenerEEG\0s_6s\',band_name{band_select},'\',listener_file_name);
%                     data_path = strcat('G:\Speaker-listener_experiment\Analysis program\Forward\speakerEEG-listenerEEG\-3s-3s\',band_name{band_select},'\',listener_file_name);
        ...
        %% ttest
        h_model_GFP = zeros(1,time_point);
        p_model_GFP = zeros(1,time_point);
        
        for time_point = 1 : length(timelag_plot)
            [h_model_GFP(time_point), p_model_GFP(time_point)] = ttest(model_attend_GFP(:,time_point), model_unattend_GFP(:,time_point));
        end
        ...
        % Diff
        subplot(132);
        temp_diff = squeeze(mean(model_attend_GFP))-squeeze(mean(model_unattend_GFP));
        plot(timelag_plot,temp_diff,'k','LineWidth',2);
        hold on; plot(timelag_plot(h_model_GFP==1),temp_diff(h_model_GFP == 1),'r*','LineWidth',2);
        title('Diff');
        
        % topoplot
        subplot(133);
        temp_topo = zeros(64,1);
        temp_topo(speaker_chn)=1;
        U_topoplot(temp_topo,layout,label66(1:64));
        title('Selected Area Topoplot')
