...
p_diff_GFP = zeros(length(speaker_chn),length(timelag_select));
% p_unattend_GFP = zeros(length(speaker_chn),length(timelag_select));

h_diff_GFP = zeros(length(speaker_chn),length(timelag_select));
...
for band_select = 1 : length(band_name)
    ...
    for chn_speaker = 1 : length(speaker_chn)
        for time_point = 1 : length(timelag_select)
            [h_diff_GFP(chn_speaker,time_point), p_diff_GFP(chn_speaker,time_point)] = ttest(model_attend_GFP_total(:,chn_speaker,time_point),model_unattend_GFP_total(:,chn_speaker,time_point));
        end
    end
    ...
    % attend p value
%     subplot(121);
    imagesc(-log10(p_diff_GFP));colorbar;colormap('jet'); 
    caxis([0, 1.3]);
%     title('difference -log10(p)');
    ylabel('Speaker Channel');xlabel('timelag(ms)');
    %             ylim([-0.03 0.03]);
...
%         xticklabels(round(timelag_plot(5:5:end)));
    ...
    save_name = strcat('mTRF SpeakerEEG forward GFP diff ttest-',band_name{band_select},'.jpg');
    ...
    %% save data
    save_name = strcat('mTRF SpeakerEEG-listenerEEG GFP diff ttest-',band_name{band_select},'.mat');
    save(save_name,'p_diff_GFP',...
       'h_diff_GFP');
end