...
%             cd(file_name);
            ...
            % R
            subplot(221);
            U_topoplot(mean(r_topoplot_attend,2),layout,label66(listener_chn));colorbar('location','EastOutside');
            title('R attended');
            
            
            % P
            subplot(223);
            U_topoplot(mean(MSE_topoplot_attend,2),layout,label66(listener_chn));colorbar('location','EastOutside');
            title('MSE attended');
            
            % R
            subplot(222);
            U_topoplot(mean(r_topoplot_unattend,2),layout,label66(listener_chn));colorbar('location','EastOutside');
            title('R unattended');
            
            
            % P
            subplot(224);
            U_topoplot(mean(MSE_topoplot_unattend,2),layout,label66(listener_chn));colorbar('location','EastOutside');
            title('MSE unattended');
            
            save_name = strcat('mTRF SpeakerEEG-ListenerEEG-',band_name{band_select},'-',chn_file_name,'.jpg');
