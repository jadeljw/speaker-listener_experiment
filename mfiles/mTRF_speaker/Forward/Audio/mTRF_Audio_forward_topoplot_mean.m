...
% ttest
h_R_attend_mean = zeros(listener_num,length(timelag),length(speaker_chn),lambda_num,length(listener_chn));
h_R_unattend_mean = zeros(listener_num,length(timelag),length(speaker_chn),lambda_num,length(listener_chn));
p_R_attend_mean = zeros(listener_num,length(timelag),length(speaker_chn),lambda_num,length(listener_chn));
p_R_unattend_mean = zeros(listener_num,length(timelag),length(speaker_chn),lambda_num,length(listener_chn));
...
%% topoplot
lambda_num = 5;
load('E:\DataProcessing\speaker-listener_experiment\Forward model\Plot\Audio theta reverse\Audio_theta_forward_zscore.mat')
...
for j = 1 : length(timelag)
    ...
    for chn_listener = 1 : length(listener_chn)
        ...
        %% plot
        % R
        R_attend_for_topoplot(chn_listener) = squeeze(mean(R_attend_mean(:,j,1,lambda_num,chn_listener),1));
        R_unattend_for_topoplot(chn_listener) = squeeze(mean(R_unattend_mean(:,j,1,lambda_num,chn_listener),1));
        
        
        % P
        P_attend_for_topoplot(chn_listener) = squeeze(mean(P_attend_mean(:,j,1,lambda_num,chn_listener),1));
        P_unattend_for_topoplot(chn_listener) = squeeze(mean(P_unattend_mean(:,j,1,lambda_num,chn_listener),1));
    ...
    save_name = strcat('mTRF Audio forward Topoplot timelag',num2str(timelag(j)),'ms.jpg');
