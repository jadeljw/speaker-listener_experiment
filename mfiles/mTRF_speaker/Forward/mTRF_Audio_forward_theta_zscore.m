...
for i = 1 : 20
    ...
    for train_index = 1 : length(data_left_theta)
        % train story
        if strcmpi('left',Space{train_index}) 
            Audio_attend{train_index} = data_left_theta{train_index}(Audio_time_index);
            
            Audio_unattend{train_index}= data_right_theta{train_index}(Audio_time_index);
        else
            Audio_attend{train_index} = data_right_theta{train_index}(Audio_time_index);
            Audio_unattend{train_index} = data_left_theta{train_index}(Audio_time_index);
        end
