% mTRF_speakerA_forward_plot_r_value

% reversed 2018.4.9
% author: LJW
% purpose: to plot mTRF crossval r value for speakerA
mkdir('SpeakerA');
cd('SpeakerA');
...
%% lambda index
lambda_index = 0 : 5 : 40;
for lambda_select  = 1 : find(lambda_index == 10)
    ...
    for band_select = 1 : length(band_name)
        ...
        %% load data
        load(strcat('E:\DataProcessing\speaker-listener_experiment\Forward model\Audio-speakerEEG\-1s-1s\SpeakerA\mTRF_sound_SpeakerA_EEG_forward_result_',band_name{band_select},'.mat'));
