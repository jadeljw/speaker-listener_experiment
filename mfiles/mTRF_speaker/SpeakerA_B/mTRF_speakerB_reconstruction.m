...
for j = 1 : length(timelag)
    ...
    saveName1 = strcat('SpeakerB Reconstruction Acc using mTRF method timelag',num2str(timelag(j)),'ms',band_name,'.jpg');
    ...
    %% save data
    saveName = strcat('mTRF_sound_SpeakerB_EEG_result+',num2str(timelag(j)),'ms',band_name,'.mat');
...
saveName = strcat('SpeakerB Reconstruction Acc using mTRF method across timelag',band_name,'.jpg');
% saveName1 = strcat('Reconstruction Accuracy using mTRF method for attend decoder.jpg');
title(saveName1(1:end-4));
legend('AudioA ');
saveas(gcf,saveName);
