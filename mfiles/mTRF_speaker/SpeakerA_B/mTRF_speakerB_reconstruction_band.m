...
for band_select = 1 : length(band_name)
    ...
    % broadband
    % data_EEG_SpeakerB = [data_speakerB_read_boradband_valid(1:14)...
    %     data_speakerB_retell_boradband_valid(1:7)...
    %     data_speakerB_retell_boradband_valid(15)...
    %     data_speakerB_retell_boradband_valid(9:14)];
    ...
    Audio_A = [data_right(1:14) data_left(15:28)];
...
%     band_name = strcat(' 64Hz 2-8Hz sound from wav SpeakerB lambda',num2str(lambda),' 10-55s');
    ...
    for j = 1 : length(timelag)
        ...
        saveName1 = strcat('SpeakerB Reconstruction Acc using mTRF method timelag',num2str(timelag(j)),'ms',bandName,'.jpg');
        ...
        save_name = strcat('SpeakerB-Topoplot-timelag ',num2str(timelag(j)),'ms.jpg');
        ...
        %% save data
        saveName = strcat('mTRF_sound_SpeakerB_EEG_result+',num2str(timelag(j)),'ms',bandName,'.mat');
    ...
    xlabel('SpeakerB'); ylabel('r value')
    saveName = strcat('SpeakerB Reconstruction Acc using mTRF method across timelag',bandName,'.jpg');
    % saveName1 = strcat('Reconstruction Accuracy using mTRF method for attend decoder.jpg');
    title(saveName1(1:end-4));
    legend('AudioA ');
    saveas(gcf,saveName);
