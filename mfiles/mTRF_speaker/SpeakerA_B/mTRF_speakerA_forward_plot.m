...
data_path = 'E:\DataProcessing\speaker-listener_experiment\Forward model\SpeakerA';
...
%     timelag = [-300 -200 -100 0 100 200 300 400];
    ...
    for j = 1 : length(timelag)
        ...
        data_name =  strcat('mTRF_sound_speakerA_EEG_forward_result_timelag',num2str(timelag(j)-150),'ms_',band_name{band_select},'.mat');
        ...
        for chn = 1 : length(chn_sel_index)
            ...
            save_name = strcat('mTRF sound speakerA EEG forward result timelag',num2str(timelag(j)-150),'ms-',band_name{band_select},'-',label66{chn_sel_index(chn)},'.jpg');
