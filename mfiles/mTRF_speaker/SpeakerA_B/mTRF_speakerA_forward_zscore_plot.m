...
data_path = 'E:\DataProcessing\speaker-listener_experiment\Forward model\Audio-speakerEEG\SpeakerA';
...
for band_select = 1 : length(band_name)
    ...
    data_name =  strcat('mTRF_sound_SpeakerA_EEG_forward_result_',band_name{band_select},'-lambda1024.mat');
    ...
    imagesc(squeeze(mean(model))');colorbar;caxis([-0.04, 0.04]);colormap('jet');
    title('TRF');ylabel('channel');xlabel('timelag(ms)');
    %             ylim([-0.03 0.03]);
    xticklabels(round(timelag_plot(5:5:end)));
    
    
    save_name = strcat('mTRF sound speakerA EEG forward result-',band_name{band_select},'.jpg');
