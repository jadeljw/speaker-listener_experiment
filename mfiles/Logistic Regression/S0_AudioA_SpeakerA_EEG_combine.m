...
% load('G:\Speaker-listener_experiment\speaker\20170619-CFY\data_preprocessing\ICA component\data_speakerA_ICA_select.mat');
% 
% select_area = 'data_speakerA_ICA';
...
for band_select = 1 : length(band_name)
    ...
    %% load speaker data
%     load('G:\Speaker-listener_experiment\speaker\20170619-CFY\data_preprocessing\ICA component\data_speakerA_ICA_component_64Hz_read_retell.mat')
    load('G:\Speaker-listener_experiment\speaker\20170619-CFY\data_preprocessing\before and afterICA\speakerA_afterICA_64Hz_read_retell.mat');
    ...
    for story_select = 1 :length(EEGBlock)
        % train story
        temp_AudioA{story_select} = zscore(Audio_A{story_select}(Audio_time)')';
    ...
    data_EEG_name = strcat('data_speakerA_total.',band_name{band_select});
    eval(strcat(data_EEG_name,'=temp_EEG'));
    data_Audio_name = strcat('Audio_A_total.',band_name{band_select});
    eval(strcat(data_Audio_name,'=temp_AudioA'));
    
end

save('data_speakerA_afterICA.mat','data_speakerA_total');
% save('Audio_A_raw.mat','Audio_A_total');
