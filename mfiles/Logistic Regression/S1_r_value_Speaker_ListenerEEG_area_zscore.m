...
%         if strcmp(band_file_name,'broadband')
%             data_temp = 'data_filtered_boradband';
%         else
...
%         end
        ...
        EEGBlock = eval(strcat(data_temp,'.trial'));
        EEGBlock = EEGBlock';
        ...
        for chn_area_select = 1 : length(chn_area_labels)
            ...
            for j = 1 : length(timelag)
                ...
                for test_index = 1 : length(EEGBlock)
                    ...
                    SpeaekerA_test = zscore(mean(data_EEG_speakerA{test_index}(speaker_chn,speaker_time_index)));
                    SpeaekerB_test = zscore(mean(data_EEG_speakerB{test_index}(speaker_chn,speaker_time_index)));
