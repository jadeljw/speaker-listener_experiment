            ...
            for j = 1 : length(timelag)
                ...
                for train_index = 1 : length(EEGBlock)
                    ...
                    if strcmpi('A',AttendTarget{train_index})
                        Audio_attend_train = mean(data_EEG_speakerA{train_index}(speaker_chn,speaker_time_index));
                        Audio_unattend_train= mean(data_EEG_speakerB{train_index}(speaker_chn,speaker_time_index));
                    else
                        Audio_attend_train = mean(data_EEG_speakerB{train_index}(speaker_chn,speaker_time_index));
                        Audio_unattend_train = mean(data_EEG_speakerA{train_index}(speaker_chn,speaker_time_index));
                    end
                ...
                for test_index = 1 : length(EEGBlock)
                    ...
                    SpeaekerA_test =mean(data_EEG_speakerA{test_index}(speaker_chn,speaker_time_index));
                    SpeaekerB_test = mean(data_EEG_speakerB{test_index}(speaker_chn,speaker_time_index));
