        ...
        for chn_area_select = 1 : length(chn_area_labels)
            ...
            for i = 1 : 20
                ...
                for time_point = 1 : length(timelag)
                    ...
                    data_path = strcat('E:\DataProcessing\speaker-listener_experiment\Logistic Regression\Speaker ListenerEEG_area\',...
...
%             save(save_name2,'R_squared_mat');
        ...
        % imagesc
        set(gcf,'outerposition',get(0,'screensize'));
        imagesc(R_squared_mat_speaker);colorbar;caxis([-0.04 0.1]);
        %     colormap('jet');
        xticks(label_select);
        xticklabels(timelag(label_select));
        ...
        save_name = strcat('Audio ListenerEEG Logstic Regresssion r^2 Result-total-',band_name{band_select},'-imagesc-',type{type_select},'-',select_area,'.jpg');
        title(save_name(1:end-4));
        saveas(gcf,save_name);
        close
