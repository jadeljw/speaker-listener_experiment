...
%%
mkdir('Speaker ListenerEEG surrogate');
cd('Speaker ListenerEEG surrogate');
%% new order
...
R_squared_mat_speaker_total = zeros(surrogate_time,length(chn_area_labels),length(timelag));
...
for band_select = 1 : length(band_name)
    ...
    for surrogate_order = 1 : surrogate_time
...
%                 cd(p(1:end-(length(chn_area_labels{chn_area_select})+1)));
            ...
            close;
            %
            R_squared_mat_speaker_total(surrogate_order,:,:) =  R_squared_mat_speaker;
...
%             cd(p(1:end-(length(type{type_select})+1)));
    ...
    %% histgram
    h = histogram(R_squared_mat_speaker_total);
    %         h.BinLimits = [-0.1 0.18];
    ylabel('count');
    xlabel('R^2 value');
    title(band_file_name);
    ...
    % save
    save_name = strcat(band_file_name,'-10 times surrogate.jpg');
    saveas(gcf,save_name);
    close;
