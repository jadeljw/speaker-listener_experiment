...
for band_select = 1 : length(band_name)
    ...
    for type_select = 1 : length(type)
        ...
        for chn = 1 : length(speaker_chn)
            ...
            for i = 1 : 20
                ...
                for time_point = 1 : length(timelag)
                    ...
                    %% load speakerEEG r value data
                    load(strcat('E:\DataProcessing\speaker-listener_experiment\Logistic Regression\Speaker-listenerEEG\',...
                        band_file_name,'\',file_name(2:end),'\',chn_file_name,'\mTRF_speakerEEG_listenerEEG_result+',label66{speaker_chn(chn)},...
                        '-timelag',num2str(timelag(time_point)),'ms-',band_file_name,'.mat'));
                    
                    %% load Audio r value data
                    ...
                    if strcmp(type{type_select},'total') == 1
                        r_value_mat = [recon_AttendDecoder_SpeakerA_corr recon_AttendDecoder_SpeakerB_corr ...
                            recon_UnattendDecoder_SpeakerA_corr recon_UnattendDecoder_SpeakerB_corr...
                            recon_AttendDecoder_AudioA_corr recon_AttendDecoder_AudioB_corr...
                            recon_UnattendDecoder_AudioA_corr recon_UnattendDecoder_AudioB_corr];
                    end
                    ...
                    %                         if strcmp(type{type_select},'total') == 1
                    %                             xticks(1:8);
                    %                             xticklabels({'Attend-SpeakerA','Attend-SpeakerB','Unattend-SpeakerA','Unattend-SpeakerB',...
                    %                                 'Attend-AudioA','Attend-AudioB','Unattend-AudioA','Unattend-AudioB'});
                    %
                    %                         end
                    %                         if strcmp(type{type_select},'diff') == 1
                    %                             xticks(1:4);
                    %                             xticklabels({'Attend-SpeakerEEG','Unattend-SpeakerEEG','Attend-Audio','Unattend-Audio'});
                    %                         end
        ...
        % imagesc
        set(gcf,'outerposition',get(0,'screensize'));
        imagesc(R_squared_mat_speaker(new_speaker_order,:));colorbar;
        %     colormap('jet');
        xticks(label_select);
        xticklabels(timelag(label_select));
        ...
        save_name = strcat('Audio Speaker-ListenerEEG Logstic Regresssion r^2 Result-All-',band_name{band_select},'-',type{type_select},'.jpg');
        title(save_name(1:end-4));
        saveas(gcf,save_name);
        close;

        save_name = strcat('Audio Speaker-ListenerEEG Logstic Regresssion r^2 Result-All-',band_name{band_select},'-',type{type_select},'mat');
