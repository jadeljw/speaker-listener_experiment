...
%     y_index_total = [];
    ...
    for jj = 1 : length(x_index_total)
        ...
        for i = 1: length(Behavioral_result_label)
            ...
            formula_right = strcat('corr(R_squared_mat_speaker_total(:,x_index_total(jj),y_index_total(jj)),',data_temp_name,');');
    ...
    for i = 1: length(Behavioral_result_label)
        ...
        save_name = strcat('R value between R^2-',Behavioral_result_label{i},'-',band_name{band_select},'.jpg');
        ...
        xlabel('timelag(ms)');
        ylabel('Speaker Channels');
        save_name = strcat('P value between R^2-',Behavioral_result_label{i},'-',band_name{band_select},'.jpg');
        title(save_name(1:end-4));
        saveas(gcf,save_name);
        close
    ...
    % save data
    save_name = strcat('Correlation between R^2-',band_name{band_select},'.mat');
