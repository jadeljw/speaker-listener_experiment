...
%       y_index_total = 1:length(timelag);
    ...
    for range_ind = 1 : length(x_index_total)
        % combine r value
        
            
            r_value_mat_GLM = zeros(listener_num,r_num);
            for raw_r_select  = 1 : length(raw_r_label)
                r_mat_data = eval(strcat('mean(Correlation_mat.',raw_r_label{raw_r_select},'(x_index_total(range_ind),:,:,y_index_total(range_ind)),3)'));
        ...
        for i = 1: length(Behavioral_result_label)
            ...
            % record into mat
               % r squared   
            formula_left = strcat('GLM_Rsquared_mat.',Behavioral_result_label{i},'(x_index_total(range_ind),y_index_total(range_ind))');
            eval(strcat(formula_left,'= glm.Rsquared.Adjusted;'));
            
              % estimate
            formula_left = strcat('GLM_Rsquared_mat_estimate.',Behavioral_result_label{i},'(x_index_total(range_ind),y_index_total(range_ind),:)');
            eval(strcat(formula_left,'= glm.Coefficients.Estimate;'));
            
        end

        disp('********************');
        disp(strcat('Timelag:',num2str(timelag(y_index_total(range_ind))),'ms'));
