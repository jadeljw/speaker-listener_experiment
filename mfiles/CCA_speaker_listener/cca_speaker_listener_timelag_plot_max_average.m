...
for jj = 1 : length(data_type)
    ...
    for i = 1 : 20
        ...
        p =strcat('E:\DataProcessing\speaker-listener_experiment\Topoplot\max\',file_name,'\',data_type_select);
        datapath = p;
        dataName = strcat('Max Acc-r rank across time-lags using CCA S-L method',bandName,'.mat');
        ...
        for j = 1 : length(timelag)
            r_rank_max_attend_acc_topoplot_speaker_mean{j}(:,i) = zscore(r_rank_max_attend_acc_topoplot_speaker{j});
            r_rank_max_attend_acc_topoplot_listener_mean{j}(:,i) = zscore(r_rank_max_attend_acc_topoplot_listener{j});
            r_rank_max_unattend_acc_topoplot_speaker_mean{j}(:,i) = zscore(r_rank_max_unattend_acc_topoplot_speaker{j});
            r_rank_max_unattend_acc_topoplot_listener_mean{j}(:,i) = zscore(r_rank_max_unattend_acc_topoplot_listener{j});
        end
    ...
    for j = 1 : length(timelag)
        figure;
        subplot(121);
        U_topoplot(abs(mean(r_rank_max_attend_acc_topoplot_listener_mean{j},2)),layout,label66(listener_chn));%plot(w_A(:,1));
        title('Listener');
        subplot(122);
        U_topoplot(abs(mean(r_rank_max_attend_acc_topoplot_speaker_mean{j},2)),layout,label66(speaker_chn));%plot(v_B(:,1));
        title('Speaker');
        ...
        figure;
        subplot(121);
        U_topoplot(abs(mean(r_rank_max_unattend_acc_topoplot_listener_mean{j},2)),layout,label66(listener_chn));%plot(w_A(:,1));
        title('Listener');
        subplot(122);
        U_topoplot(abs(mean(r_rank_max_unattend_acc_topoplot_speaker_mean{j},2)),layout,label66(speaker_chn));%plot(v_B(:,1));
        title('Speaker');
    ...
    saveName3 =strcat('Average Max Acc-r rank acrcoss time-lags using CCA S-L method',data_type_select,'.jpg');
    ...
    saveName =strcat('Max Acc-r rank across time-lags using CCA S-L method',data_type_select,'.mat');
